...
(function () {
  ...
  L.screens.paymentForm = {
    ...
    render: function (view, kind, partyId) {
      ...
      if (partyId && S.data.party === undefined) {
        var url = (kind === 'payment_out' ? '/api/suppliers/' : '/api/customers/') + Number(partyId);
  ...
  L.paymentPick = function (kind) {
    L.pickParty(kind === 'payment_out' ? 'supplier' : 'customer', function (party) {
      S.data.party = party;
      L.render();
    });
  };
  ...
  L.paymentSave = function (kind) {
    ...
    if (S.data.party) {
      body.party_type = kind === 'payment_out' ? 'supplier' : 'customer';
