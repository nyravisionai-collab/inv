...
(function () {
  ...
  function drawList(box, type) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var party = purchase ? d.party_name : d.party_name;
      var target = purchase
  ...
  function renderDetail(view, id, purchase) {
    ...
    h += '</table></div><hr class="sp" />'
      ...
      + (d.profit !== undefined ? '<div class="kv"><span class="k">' + esc(t('profit')) + '</span><span class="v" style="color:' + (d.profit >= 0 ? 'green' : 'red') + ';">' + esc(L.money(d.profit)) + '</span></div>' : '')
    ...
    var upiId = (window.Lite.S.settings && window.Lite.S.settings.upi_id) || '';
    var upiQr = (!purchase && upiId && L.num(d.balance_amount) > 0)
      ? '<div style="text-align:center;margin-top:15px;padding:10px;background:#f9f9f9;border-radius:8px;">'
        + '<div style="font-size:11px;margin-bottom:5px;">' + esc(t('scanToPay')) + '</div>'
        + '<img src="https://api.qrserver.com/v1/create-qr-code/?size=120x120&data='
        + encodeURIComponent('upi://pay?pa=' + upiId + '&pn=' + encodeURIComponent(window.Lite.S.settings.company_name || 'Business') + '&am=' + d.balance_amount + '&cu=INR')
        + '" style="width:120px;height:120px;" />'
        + '<div style="font-size:10px;margin-top:5px;">' + esc(upiId) + '</div></div>'
      : '';

    h += upiQr + '<div class="btnrow">'
      ...
      + '<div class="btnhalf">' + (purchase
        ? '<button type="button" class="btn grey" onclick="Lite.go(\'paymentForm\',\'payment_out\','
          + Number(d.party_id || 0) + ')">' + esc(t('recordPayment')) + '</button>'
  ...
  L.screens.docForm = {
    ...
    render: function (view, type) {
      ...
      var h = '<div class="card">'
        ...
        + (purchase
          ? '<label class="fld" for="df_sinv">' + esc(t('partyInvoice')) + '</label>'
            + '<input type="text" id="df_sinv" value="' + esc(draft.partyInvoice)
            + '" onchange="Lite.docField(\'partyInvoice\',this.value)" />'
          : '')
  ...
  function drawParty() {
    ...
    var lbl = purchase ? t('party') : t('party');
    if (draft.party) {
  ...
  L.docPickParty = function () {
    L.pickParty(isPurchase(draft.type) ? 'party' : 'party', function (party) {
      draft.party = party;
      L.go('docForm', draft.type, null, true);
    });
  };
  ...
  L.docSave = function () {
    ...
    if (purchase) {
      ...
      if (draft.party) body.party_id = draft.party.id;
      if (L.trim(draft.partyInvoice)) body.party_invoice = L.trim(draft.partyInvoice);
    } else {
