 ...
 * quantity/price/discount, a customer, bill-level discount in ₹ or %, all
...
(function () {
  ...
  function drawCustomer() {
    var box = L.$('posCustomer');
    if (!box) return;
    if (cart.customer) {
      box.innerHTML = '<div class="card"><div class="rowl"><b>' + esc(cart.customer.name) + '</b>'
        + (cart.customer.phone ? '<div class="muted">' + esc(cart.customer.phone) + '</div>' : '')
        + '</div><div class="rowr"><button type="button" class="btn grey small"'
        + ' onclick="Lite.posPickCustomer()">' + esc(t('changeParty')) + '</button></div></div>';
    } else {
      box.innerHTML = '<div class="card"><button type="button" class="btn grey"'
        + ' onclick="Lite.posPickCustomer()">' + esc(t('walkIn')) + ' — ' + esc(t('selectParty')) + '</button></div>';
    }
  }

  L.posPickCustomer = function () {
    L.pickParty('customer', function (party) {
      cart.customer = party;
  ...
  L.posSave = function () {
    ...
    var body = {
      invoice_type: 'pos',
      status: 'completed',
      customer_id: cart.customer ? cart.customer.id : undefined,
      items: items,
