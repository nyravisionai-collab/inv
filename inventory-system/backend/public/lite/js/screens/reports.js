...
(function () {
  ...
  var REPORTS = {
    ...
    partys: { label: 'rPartys', url: '/api/reports/partys', dated: false, pdf: 'partys', draw: drawParties },
    partys: { label: 'rPartys', url: '/api/reports/partys', dated: false, pdf: 'partys', draw: drawParties },
    ...
    'party-profit': { label: 'rPartyProfit', url: '/api/reports/party-profit', dated: true, pdf: 'party-profit', draw: drawPartyProfit },
  ...
  var ORDER = ['profit-loss', 'balance-sheet', 'gst', 'sales', 'purchases', 'expenses',
    'stock', 'warehouse-stock', 'expiry', 'partys', 'partys', 'outstanding',
    'product-profit', 'party-profit'];
      ...
      var head = '<div class="card">'
        ...
        + (r.group
          ? '<label class="fld" for="rpg">' + esc(t('groupBy')) + '</label>'
            ...
            + '<option value="party"' + (group === 'party' ? ' selected="selected"' : '') + '>' + esc(t('gParty')) + '</option>'
  ...
  function drawSales(d) {
    ...
    for (var i = 0; i < (d.rows || []).length; i++) {
      ...
      else if (d.group_by === 'party') rows.push([esc(r.name || t('walkIn')), String(r.invoices), esc(L.money(r.total))]);
      else rows.push([esc(L.dstr(r.date)), String(r.invoices), esc(L.money(r.total))]);
    }
    if (!rows.length) return h + L.emptyCard(t('noData'));
    var first = d.group_by === 'product' ? t('name') : (d.group_by === 'party' ? t('party') : t('date'));
  ...
  function drawPurchases(d) {
    ...
    for (var i = 0; i < (d.rows || []).length; i++) {
      ...
      rows.push([esc(r.bill_number) + '<div class="muted">' + esc(L.dstr(r.date)) + '</div>',
        esc(r.party || '—'), esc(L.money(r.total))]);
    }
    if (!rows.length) return h + L.emptyCard(t('noData'));
    return h + table([{ label: t('billNo') }, { label: t('party') }, { label: t('total'), n: true }], rows);
  }
  ...
  function drawOutstanding(d) {
    var h = '<div class="card">'
      + L.kv(t('receivables'), L.money(d.partyOutstanding), 'red')
      + L.kv(t('payables'), L.money(d.partyPayable), 'red')
      + '</div>';
    var rows = [], i;
    for (i = 0; i < (d.partys || []).length; i++) {
      rows.push([esc(d.partys[i].name), esc(L.money(d.partys[i].outstanding))]);
    }
    if (rows.length) {
      h += L.sectionTitle(t('partys'))
        + table([{ label: t('name') }, { label: t('outstanding'), n: true }], rows);
    }
    var srows = [];
    for (i = 0; i < (d.partys || []).length; i++) {
      srows.push([esc(d.partys[i].name), esc(L.money(d.partys[i].payable))]);
    }
    if (srows.length) {
      h += L.sectionTitle(t('partys'))
  ...
  function drawPartyProfit(d) {
    var rows = [];
    for (var i = 0; i < (d.rows || []).length; i++) {
      var r = d.rows[i];
      rows.push([esc(r.party_name), String(r.invoices), esc(L.money(r.sales)), esc(L.money(r.profit))]);
    }
    if (!rows.length) return L.emptyCard(t('noData'));
    return table([{ label: t('party') }, { label: t('invoices'), n: true },
      { label: t('sales'), n: true }, { label: t('profit'), n: true }], rows);
  }
