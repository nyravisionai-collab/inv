/**
 * Partys and partys: list, create/edit, ledger, outstanding and the
...
(function () {
  ...
  function base(kind) { return kind === 'party' ? '/api/partys' : '/api/partys'; }
  function label(kind) { return kind === 'party' ? t('partys') : t('partys'); }
      ...
      L.api('GET', base(kind) + '/outstanding', null, function (err, data) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          h += '<div class="btnrow">'
            ...
            + (kind === 'party' ? 'payment_out' : 'payment_in') + '\',' + Number(p.id) + ')">'
  ...
  L.screens.partyDetail = {
    title: function (kind) { return kind === 'party' ? t('party') : t('party'); },
    render: function (view, kind, id) {
      ...
      var h = '<div class="card">'
        ...
        + (kind === 'party' ? 'payment_out' : 'payment_in') + '\',' + Number(id) + ')">'
  ...
  L.screens.partyForm = {
    title: function (kind, id) {
      return (id ? t('edit') : t('add')) + ' — ' + (kind === 'party' ? t('party') : t('party'));
    },
    render: function (view, kind, id) {
